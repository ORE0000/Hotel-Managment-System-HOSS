...
import { Enquiry ,BillData} from '../types';
import {
  useState,
  useMemo,
  useCallback,
...
const EnquiryPage: React.FC = () => {
  ...
  const [hasAutoNavigated, setHasAutoNavigated] = useState(false);
    ...
    const mapToBillData = (booking: Enquiry): Partial<BillData> => ({
  ...
  // ---------------------------------------------------------------------
  // AUTO JUMP TO TODAY OR NEAREST DATE — RUN ONLY ON FIRST DATA LOAD
  // ---------------------------------------------------------------------
  useEffect(() => {
    if (!enquiries || !enquiries.length || hasAutoNavigated) return;

    const today = new Date().toISOString().split('T')[0];
    const todayTime = new Date(today).getTime();

    // 1️⃣ Try exact match
    let targetIndex = enquiries.findIndex((item) => item.checkIn === today);

    // 2️⃣ Nearest Past / Future if today not found
    if (targetIndex === -1) {
      let nearestPast = { index: -1, diff: Infinity };
      let nearestFuture = { index: -1, diff: Infinity };

      enquiries.forEach((item, idx) => {
    ...
    // If a valid index exists → jump to page
    if (targetIndex !== -1) {
      const targetPage = Math.floor(targetIndex / itemsPerPage) + 1;
      setPage(targetPage);
      setHasAutoNavigated(true); // ensure NEVER runs again
    }
  }, [enquiries, hasAutoNavigated, itemsPerPage]);
