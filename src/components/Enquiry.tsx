...
const EnquiryPage: React.FC = () => {
  ...
  const { data: hotels, error: hotelsError, isError: isHotelsError } = useQuery<string[]>({
    queryKey: ['hotels'],
    queryFn: fetchHotels,
    retry: 2,
  });
  ...
  const printEnquiryDetails = async () => {
    ...
    try {
      ...
      pdf.save(`enquiry_${selectedEnquiry.guestName}_${selectedEnquiry.checkIn}.pdf`);
  ...
  const printSummaryDetails = async () => {
    ...
    try {
      ...
      pdf.save(`summary_${selectedEnquiry.guestName}_${selectedEnquiry.checkIn}.pdf`);
...
const downloadPDF = async () => {
  ...
  try {
    ...
    const indices = selectedRows.size > 0 ? Array.from(selectedRows) : filteredData.map((_, idx) => idx);

    for (let i = 0; i < indices.length; i++) {
      const originalIndex = indices[i]; // Index in filteredData
      const element = document.getElementById(`enquiry-preview-${originalIndex}`);
      if (!element) {
  ...
  return (
    <div className="space-y-8 p-6 w-full">
      ...
      <div id="pdf-single-preview" style={{ display: 'none', position: 'fixed', top: 0, left: 0 }}>
        {selectedEnquiry && <EnquiryPreview enquiryData={selectedEnquiry} />}
      </div>
      <div id="pdf-single-summary-preview" style={{ display: 'none', position: 'fixed', top: 0, left: 0 }}>
        {selectedEnquiry && <div className="summary-single-content"><EnquirySummaryPreview enquiryData={[selectedEnquiry]} /></div>}
      </div>
      ...
      <Modal
        ...
        className="modal-content max-w-2xl mx-auto my-8 p-6 bg-[var(--card-bg)] rounded-xl shadow-lg neumorphic-card"
        ...
        {selectedEnquiry && (
          <div className="relative">
            ...
            <h2 className="text-2xl font-bold text-gradient mb-6">
              Enquiry Details - {selectedEnquiry.guestName}
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-[var(--text-secondary)]">
              <div>
                ...
                <p>
                  ...
                  {selectedEnquiry.roomName.doubleBed}, Triple Bed=
                  {selectedEnquiry.roomName.tripleBed}, Four Bed=
                  {selectedEnquiry.roomName.fourBed}, Extra Bed=
                  {selectedEnquiry.roomName.extraBed}, Kitchen=
                  {selectedEnquiry.roomName.kitchen}
                </p>
                <p>
                  <span className="font-medium">Room Rates:</span> Double Bed=₹
                  {selectedEnquiry.roomRent.doubleBed.toLocaleString()}, Triple Bed=₹
                  {selectedEnquiry.roomRent.tripleBed.toLocaleString()}, Four Bed=₹
                  {selectedEnquiry.roomRent.fourBed.toLocaleString()}, Extra Bed=₹
                  {selectedEnquiry.roomRent.extraBed.toLocaleString()}, Kitchen=₹
                  {selectedEnquiry.roomRent.kitchen.toLocaleString()}
                </p>
                <p>
                  <span className="font-medium">Discount:</span> Double Bed=₹
                  {selectedEnquiry.discount.doubleBed.toLocaleString()}, Triple Bed=₹
                  {selectedEnquiry.discount.tripleBed.toLocaleString()}, Four Bed=₹
                  {selectedEnquiry.discount.fourBed.toLocaleString()}, Extra Bed=₹
                  {selectedEnquiry.discount.extraBed.toLocaleString()}, Kitchen=₹
                  {selectedEnquiry.discount.kitchen.toLocaleString()}
                </p>
              </div>
              <div>
                <p>
                ...
                <p>
                  <span className="font-medium">Date:</span> {selectedEnquiry.date}
                </p>
                <p>
                  <span className="font-medium">To Account:</span>{' '}
                  {selectedEnquiry.toAccount}
                </p>
                <p>
                  <span className="font-medium">Scheme:</span> {selectedEnquiry.scheme}
                </p>
                <p>
                  <span className="font-medium">Status:</span>{' '}
                  <span className={`badge badge-${selectedEnquiry.status.toLowerCase()}`}>
            ...
            <div className="mt-6 flex flex-wrap gap-3 sm:gap-4 justify-center">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
              >
                <FiFileText size={16} /> Summary PDF
              </motion.button>
            </div>
          </div>
        )}
