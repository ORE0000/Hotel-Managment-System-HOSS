...
import { fetchEnquiryData, fetchHOSSBookings, submitData } from '../services/ApiService';
...
import { isToday, isYesterday, differenceInDays, startOfWeek, endOfWeek } from 'date-fns';
...
const Dashboard: React.FC = () => {
  ...
  const calculateTrends = useCallback(() => {
    ...
    const calculateOccupancyForPeriod = (start: Date, end: Date) => {
      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        const dailyBookings = hossBookings.filter(booking => {
        ...
        dailyBookings.forEach(booking => {
          const hotels = booking.hotelName?.split('/') || ['HOSS'];
          if (hotels.includes('HOSS')) {
            const rooms = 
              (parseInt(booking.db || '0') || 0) +
              (parseInt(booking.tb || '0') || 0) +
              (parseInt(booking.fb || '0') || 0);
            dailyOccupiedRooms += hotels.length === 1 ? rooms : rooms / hotels.length;
          }
        });
    ...
    const prevWeekOccupancy = calculateOccupancyForPeriod(prevWeekStart, prevWeekEnd);
    
    const occupancyDiff = Math.round(currentWeekOccupancy - prevWeekOccupancy);
    const occupancyTrend = occupancyDiff > 0 
    ...
    const todayRevenue = hossBookings
      .filter(b => isToday(new Date(b.checkIn)) && b.status !== 'Cancelled')
      .reduce((sum, b) => sum + (b.totalBill || 0), 0);
    const yesterdayRevenue = hossBookings
      .filter(b => isYesterday(new Date(b.checkIn)) && b.status !== 'Cancelled')
      .reduce((sum, b) => sum + (b.totalBill || 0), 0);
    const revenuePercentChange = yesterdayRevenue !== 0 
      ? Math.round(((todayRevenue - yesterdayRevenue) / yesterdayRevenue) * 100)
      : todayRevenue > 0 ? 100 : 0;
    const revenueTrend = revenuePercentChange > 0 
  ...
  const calculateOccupancyRate = useCallback(() => {
    ...
    const activeBookings = hossBookings.filter((booking) => {
      if (booking.status === 'Cancelled') return false;
      const checkIn = new Date(booking.checkIn);
      const checkOut = new Date(booking.checkOut);
    ...
    activeBookings.forEach((booking) => {
      ...
      const db = parseInt(booking.db || '0') || 0;
      const tb = parseInt(booking.tb || '0') || 0;
      const fb = parseInt(booking.fb || '0') || 0;
      const extra = parseInt(booking.extra || '0') || 0;
  ...
  const SkeletonLoader = ({ count, type }: { count: number; type: 'card' | 'table' | 'calendar' }) => {
    if (type === 'card') {
      return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[...Array(count)].map((_, idx) => (
            <div key={idx} className="glass-card p-5 rounded-2xl neumorphic-card animate-pulse">
    ...
    if (type === 'table') {
      return (
        <div className="space-y-4">
          {[...Array(count)].map((_, idx) => (
            <div key={idx} className="flex items-center p-4 border border-[var(--border-color)] rounded-xl animate-pulse">
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        ...
        <div className="mt-4 md:mt-0 flex items-center space-x-3">
          <motion.button
            ...
            style={{ boxShadow: '4px 4px 8px rgba(0, 0, 0, 0.1), -4px -4px 8px rgba(255, 255, 255, 0.2)' }}
          >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            animate={refreshMutation.isPending ? { scale: [1, 1.05, 1], transition: { repeat: Infinity, duration: 0.8 } } : {}}
          ...
          >
            <div className="relative flex items-center">
              ...
              <input
                ...
                style={{ 
                  boxShadow: 'inset 2px 2px 4px rgba(0, 0, 0, 0.1), inset -2px -2px 4px rgba(255, 255, 255, 0.2)',
                  border: 'none',
                }}
              />
              {searchQuery && (
          ...
          {[
            {
              ...
              value: hossBookings?.filter((b) => isToday(new Date(b.checkIn)) && b.status !== 'Cancelled').length || '0',
              icon: <FiCheckCircle size={24} />,
              trend: trends.checkInTrend,
              trendColor: trends.checkInTrend.includes('+') ? "text-green-400" : trends.checkInTrend.includes('-') ? "text-red-400" : "text-gray-400",
              trendIcon: <FiCheckCircle size={12} />,
              bg: "bg-[var(--icon-bg-indigo)]",
            },
            {
              title: "Today's Check-outs",
              value: hossBookings?.filter((b) => isToday(new Date(b.checkOut)) && b.status !== 'Cancelled').length || '0',
              icon: <FiLogOut size={24} />,
              trend: trends.checkOutTrend,
              trendColor: trends.checkOutTrend.includes('+') ? "text-green-400" : trends.checkOutTrend.includes('-') ? "text-red-400" : "text-gray-400",
            ...
            },
            {
              ...
              trendColor: trends.occupancyTrend.includes('+') ? "text-green-400" : trends.occupancyTrend.includes('-') ? "text-red-400" : "text-gray-400",
            ...
            }] : []),
            {
              ...
              value: `₹${(
                hossBookings
                  ?.filter((b) => isToday(new Date(b.checkIn)) && b.status !== 'Cancelled')
                  .reduce((sum, b) => sum + (b.totalBill || 0), 0) || 0
              ).toLocaleString()}`,
              icon: <FiDollarSign size={24} />,
              trend: trends.revenueTrend,
              trendColor: trends.revenueTrend.includes('+') ? "text-green-400" : trends.revenueTrend.includes('-') ? "text-red-400" : "text-gray-400",
            ...
            >
              ...
              <div className="flex items-center justify-between relative z-10">
                ...
                <div className={`w-14 h-14 rounded-xl ${stat.bg} flex items-center justify-center shadow-lg`}>
                  {stat.icon}
                </div>
              </div>
              <div className={`mt-3 text-sm ${stat.trendColor} flex items-center relative z-10`}>
                {stat.trendIcon}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card p-6 rounded-2xl neumorphic-card">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-[var(--text-primary)]">Booking Calendar</h2>
          ...
          {isBookingsLoading ? (
            ...
            <div className="space-y-4">
              {recentBookings().length > 0 ? (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <h4 className="font-medium text-[var(--text-primary)]">{booking.guestName}</h4>
                ...
                <p className="text-[var(--text-secondary)] text-center">No bookings for today</p>
              )}
            </div>
          )}
        </div>
      ...
      <div className="glass-card p-6 rounded-2xl neumorphic-card">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-[var(--text-primary)]">Financial Overview</h2>
          <div className="flex space-x-3">
            <select className="input-field text-sm px-4 py-2 rounded-full neumorphic-input">
            ...
            <motion.button
              className="btn-secondary flex items-center gap-2 px-4 py-2 rounded-full"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-[var(--text-secondary)] font-medium">{item.title}</p>
                    <h3 className="text-2xl font-bold mt-1 text-[var(--text-primary)]">{item.value}</h3>
                  </div>
                  <div className={`w-12 h-12 rounded-xl ${item.bg} flex items-center justify-center shadow-lg`}>
                    {item.icon}
                  </div>
                </div>
        ...
        <div className="mt-6 h-72 bg-[var(--card-bg)] rounded-xl flex items-center justify-center neumorphic-inner">
          <p className="text-[var(--text-secondary)]">Revenue Chart Coming Soon</p>
        </div>
      </div>
      ...
      <div className="glass-card p-6 rounded-2xl neumorphic-card">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-[var(--text-primary)]">Recent Enquiries</h2>
          <Link to="/enquiry">
            <motion.button
              className="flex items-center gap-2 px-4 py-2 bg-[var(--button-bg)] text-[var(--text-primary)] rounded-xl neumorphic-button font-semibold border border-[var(--border-color)] hover:shadow-[0_0_15px_var(--glow-color)] transition-all duration-300"
              whileHover={{ scale: 1.05 }}
            ...
            <table className="modern-table w-full">
              ...
              <tbody>
                {recentEnquiries.length > 0 ? (
                  recentEnquiries.map((enquiry, index) => (
                    ...
                    >
                      ...
                      <td data-label="Double Bed">
                        <div className="text-sm">{enquiry.roomName.doubleBed}</div>
                      </td>
                      <td data-label="Triple Bed">
                        <div className="text-sm">{enquiry.roomName.tripleBed}</div>
                      </td>
                      <td data-label="Four Bed">
                        <div className="text-sm">{enquiry.roomName.fourBed}</div>
                      </td>
                      <td data-label="Extra Bed">
                        <div className="text-sm">{enquiry.roomName.extraBed}</div>
                      </td>
                      <td data-label="Kitchen">
                        <div className="text-sm">{enquiry.roomName.kitchen}</div>
                      </td>
                      <td data-label="Bill Amount">
                        <div className="text-sm">₹{enquiry.billAmount.toLocaleString()}</div>
                      </td>
                      <td data-label="Actions">
                        <div className="flex space-x-3">
                          ...
                          <motion.button
                            ...
                            whileHover={{ scale: 1.1 }}
                          >
                            {enquiry.status === 'Converted' ? 'View' : 'Convert'}
                          </motion.button>
                          <motion.button
                  ...
                  <tr>
                    <td colSpan={13} className="p-6 text-center text-[var(--text-secondary)]">
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6 border-b border-[var(--border-color)] pb-4">
                <h3 className="text-2xl font-semibold text-gradient">Create New Booking</h3>
                <motion.button
        ...
        {isSuccessDialogOpen && isPageLoaded && (
          ...
          >
            ...
            <motion.div
              ...
              transition={{ duration: 0.4, ease: 'easeOut' }}
              style={{ boxShadow: '0 8px 32px rgba(0, 128, 0, 0.4), inset 2px 2px 4px rgba(255, 255, 255, 0.2)' }}
            >
              ...
              <h3 className="text-xl font-semibold text-center">Booking Created Successfully!</h3>
      ...
      <AnimatePresence>
        {isAllBookingsModalOpen && isPageLoaded && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6 border-b border-[var(--border-color)] pb-4">
                <h3 className="text-2xl font-semibold text-gradient">Today's Bookings</h3>
                <motion.button
                  className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
                  whileHover={{ scale: 1.2 }}
                  onClick={() => setIsAllBookingsModalOpen(false)}
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-[var(--text-secondary)] mt-2">
                            ...
                            <span className="text-emerald-400">₹{booking.advance?.toLocaleString() || '0'}</span>
                           </div>
                          <div className="text-xs text-[var(--text-secondary)] mt-1">
                            <span className="font-medium">Total Bill:</span>{' '}
                            <span className="text-blue-400">₹{booking.totalBill?.toLocaleString() || '0'}</span>
                          </div>
                        </div>
                      </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-semibold text-[var(--text-primary)]">Booking Details</h3>
                <motion.button
                  className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
                  whileHover={{ scale: 1.2 }}
                  onClick={() => setIsBookingDetailModalOpen(false)}
              ...
              <div className="space-y-3 text-sm text-[var(--text-primary)]">
                ...
                <p>
                  ...
                  <span className={`badge badge-${selectedBooking.status.toLowerCase().replace(' ', '-')}`}>
        ...
        {isEnquiryDetailModalOpen && isPageLoaded && selectedEnquiry && (
          ...
          >
            ...
            <motion.div
              ...
              exit={{ scale: 0.9, opacity: 0, y: 50 }}
              transition={{ duration: 0.4, ease: 'easeOut' }}
              style={{ boxShadow: '0 8px 32px rgba(0, 0, 128, 0.4), inset 2px 2px 4px rgba(255, 255, 255, 0.2)' }}
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-semibold text-[var(--text-primary)]">Enquiry Details</h3>
                <motion.button
                  className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
                  whileHover={{ scale: 1.2 }}
                  onClick={() => setIsEnquiryDetailModalOpen(false)}
              ...
              <div className="space-y-3 text-sm text-[var(--text-primary)]">
                ...
                <p><span className="font-medium">Double Bed:</span> {selectedEnquiry.roomName.doubleBed}</p>
                <p><span className="font-medium">Triple Bed:</span> {selectedEnquiry.roomName.tripleBed}</p>
                <p><span className="font-medium">Four Bed:</span> {selectedEnquiry.roomName.fourBed}</p>
                <p><span className="font-medium">Extra Bed:</span> {selectedEnquiry.roomName.extraBed}</p>
                <p><span className="font-medium">Kitchen:</span> {selectedEnquiry.roomName.kitchen}</p>
                <p>
                  <span className="font-medium">Bill Amount:</span>{' '}
                  <span className="text-blue-400">₹{selectedEnquiry.billAmount.toLocaleString()}</span>
                </p>
                <p>
                  <span className="font-medium">Status:</span>{' '}
                  <span className={`badge badge-${selectedEnquiry.status.toLowerCase().replace(' ', '-')}`}>
