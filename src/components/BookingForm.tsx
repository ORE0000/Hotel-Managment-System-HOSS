...
import { FiUser, FiCalendar, FiCreditCard, FiCheckCircle, FiCopy, FiXCircle, FiRefreshCw, FiEdit } from "react-icons/fi";
...
const bookingSchema = z.object({
  dateBooked: z.string().regex(/^\d{2}-\d{2}-\d{4}$/, "Date Booked must be in DD-MM-YYYY format"),
  guestName: z.string().min(1, "Guest Name is required"),
  contact: z.string().optional().default(""),
  plan: z.enum(["MAP", "EP"]),
  hotel: z.string().min(1, "Hotel is required"),
  customHotel: z.string().optional().default(""),
  checkIn: z.string().regex(/^\d{2}-\d{2}-\d{4}$/, "Check-In date must be in DD-MM-YYYY format"),
  checkOut: z.string().regex(/^\d{2}-\d{2}-\d{4}$/, "Check-Out date must be in DD-MM-YYYY format"),
  ...
  (data) => {
    if (data.checkIn && data.checkOut) {
      const [checkInDay, checkInMonth, checkInYear] = data.checkIn.split("-").map(Number);
      const [checkOutDay, checkOutMonth, checkOutYear] = data.checkOut.split("-").map(Number);
...
const BookingForm: React.FC = () => {
  ...
  const [calculated, setCalculated] = useState({ days: 0, bill: "0.00", due: "0.00" });
  const [showClearDialog, setShowClearDialog] = useState(false);
  const [showSubmitDialog, setShowSubmitDialog] = useState(false);
  const [formDataToSubmit, setFormDataToSubmit] = useState<FormData | null>(null);

  const { register, handleSubmit, reset, getValues, setValue, trigger, formState: { errors, isValid } } = useForm<FormData>({
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.success) {
        toast.success("Booking submitted successfully! Data synced with Excel.");
        reset();
  ...
  const calculateDays = useCallback((data: FormData) => {
    if (data.checkIn && data.checkOut) {
      const [checkInDay, checkInMonth, checkInYear] = data.checkIn.split("-").map(Number);
      const [checkOutDay, checkOutMonth, checkOutYear] = data.checkOut.split("-").map(Number);
  ...
  const calculateBill = useCallback((data: FormData) => {
  ...
  const dueAmount = useCallback((data: FormData) => {
  ...
  return (
    ...
    >
        ...
        {showClearDialog && (
          ...
          >
            <motion.div
              ...
              transition={{ duration: 0.5, ease: "easeOut", animation: "dialogPulse" }}
          ...
          >
            ...
            >
              <div className="booking-form-dialog-content">
                ...
                <p className="text-[var(--text-secondary)] mb-6">
                  Are you sure you want to submit this booking? Please review all details before proceeding.
                </p>
                <div className="booking-form-dialog-buttons">
                  <motion.button
      ...
      >
        ...
        <p className="text-sm text-[var(--text-secondary)] mt-1">Seamlessly create and sync bookings with Excel</p>
      </motion.header>
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            ...
            {[
              ...
              { label: "Guest Name", name: "guestName", type: "text", required: true, icon: <FiUser /> },
              { label: "Contact", name: "contact", type: "text", icon: <FiUser /> },
              {
                label: "Plan",
                name: "plan",
                type: "select",
              ...
              },
              {
                ...
                name: "hotel",
                type: "select",
                options: hotelsLoading ? ["Loading..."] : [...(hotels || []), "Other"],
              ...
              { label: "PAX (Guests)", name: "pax", type: "number", min: "0", icon: <FiUser /> },
              {
                label: "Status",
                name: "status",
                type: "select",
                ...
                <div className="flex items-center gap-2">
                  ...
                  {field.type === "select" ? (
                    <select
                      ...
                      onChange={(e) => {
                        setValue(field.name as keyof FormData, e.target.value, { shouldValidate: true });
                    ...
                    <DatePicker
                      id={field.name}
                      selected={parseDDMMYYYYToDate(getValues(field.name as keyof FormData))}
                      onChange={(date: Date) => {
                        setValue(field.name as keyof FormData, formatDateToDDMMYYYY(date), { shouldValidate: true });
                      }}
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { label: "Quantity of Double Bed (DB)", name: "db", type: "number", min: "0", icon: <FaBed /> },
                  { label: "Quantity of Triple Bed (TB)", name: "tb", type: "number", min: "0", icon: <FaBed /> },
                  { label: "Quantity of Four Bed (FB)", name: "fb", type: "number", min: "0", icon: <FaBed /> },
                  { label: "Quantity of Extra Bed", name: "extra", type: "number", min: "0", icon: <FaBed /> },
                  { label: "Quantity of Kitchen", name: "kitchen", type: "number", min: "0", icon: <FaBed /> },
                ].map((field, index) => (
                  <motion.div
                    key={field.name}
                    initial={{ opacity: 0, y: 10 }}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { label: "Double Bed Rate (₹)", name: "dbRate", type: "number", step: "0.01", min: "0", icon: <FiCreditCard /> },
                  { label: "Triple Bed Rate (₹)", name: "tbRate", type: "number", step: "0.01", min: "0", icon: <FiCreditCard /> },
                  { label: "Four Bed Rate (₹)", name: "fbRate", type: "number", step: "0.01", min: "0", icon: <FiCreditCard /> },
                  { label: "Extra Bed Rate (₹)", name: "extraRate", type: "number", step: "0.01", min: "0", icon: <FiCreditCard /> },
                  { label: "Kitchen Rate (₹)", name: "kitchenRate", type: "number", step: "0.01", min: "0", icon: <FiCreditCard /> },
                ].map((field, index) => (
                  <motion.div
                    key={field.name}
                    initial={{ opacity: 0, y: 10 }}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { label: "Double Bed Discount (%)", name: "dbDiscount", type: "number", step: "0.01", min: "0", icon: <FiCreditCard /> },
                  { label: "Triple Bed Discount (%)", name: "tbDiscount", type: "number", step: "0.01", min: "0", icon: <FiCreditCard /> },
                  { label: "Four Bed Discount (%)", name: "fbDiscount", type: "number", step: "0.01", min: "0", icon: <FiCreditCard /> },
                  { label: "Extra Bed Discount (%)", name: "extraDiscount", type: "number", step: "0.01", min: "0", icon: <FiCreditCard /> },
                  { label: "Kitchen Discount (%)", name: "kitchenDiscount", type: "number", step: "0.01", min: "0", icon: <FiCreditCard /> },
                ].map((field, index) => (
                  <motion.div
                    key={field.name}
                    initial={{ opacity: 0, y: 10 }}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { label: "Advance (₹)", name: "advance", type: "number", step: "0.01", min: "0", icon: <FiCreditCard /> },
                  {
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { label: "Total Days", value: calculated.days, icon: <FiCalendar />, color: "bg-[var(--icon-bg-indigo)]" },
              { label: "Total Bill", value: `₹${calculated.bill}`, icon: <FiCreditCard />, color: "bg-[var(--icon-bg-green)]" },
              { label: "Due Amount", value: `₹${calculated.due}`, icon: <FiCreditCard />, color: "bg-[var(--icon-bg-yellow)]" },
            ].map((item, index) => (
              <motion.div
                key={index}
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="p-4 rounded-lg bg-[var(--card-bg)] border border-[var(--border-color)]">
              <h4 className="text-sm font-medium text-[var(--text-secondary)] mb-2">Room Details</h4>
              <div className="space-y-1">
                <p className="text-sm"><span className="font-medium">DB:</span> {getValues("db") || "0"}</p>
                <p className="text-sm"><span className="font-medium">TB:</span> {getValues("tb") || "0"}</p>
                <p className="text-sm"><span className="font-medium">FB:</span> {getValues("fb") || "0"}</p>
              </div>
            </div>

            <div className="p-4 rounded-lg bg-[var(--card-bg)] border border-[var(--border-color)]">
              <h4 className="text-sm font-medium text-[var(--text-secondary)] mb-2">Payment Details</h4>
              <div className="space-y-1">
                <p className="text-sm"><span className="font-medium">Advance:</span> ₹{getValues("advance") || "0"}</p>
                <p className="text-sm"><span className="font-medium">Due:</span> ₹{calculated.due}</p>
                <p className="text-sm"><span className="font-medium">Method:</span> {getValues("paymentMethod")}</p>
              </div>
            </div>

            <div className="p-4 rounded-lg bg-[var(--card-bg)] border border-[var(--border-color)]">
              <h4 className="text-sm font-medium text-[var(--text-secondary)] mb-2">Stay Details</h4>
              <div className="space-y-1">
                <p className="text-sm"><span className="font-medium">Check-In:</span> {getValues("checkIn") || "-"}</p>
                <p className="text-sm"><span className="font-medium">Check-Out:</span> {getValues("checkOut") || "-"}</p>
                <p className="text-sm"><span className="font-medium">Days:</span> {calculated.days}</p>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <div className="mt-8 booking-form-buttons flex flex-col sm:flex-row justify-end gap-4">
          ...
          >
            <motion.span
              ...
              transition={{ repeat: refreshMutation.isPending ? Infinity : 0, duration: 1 }}
            >
              <FiRefreshCw size={20} />
            </motion.span>
            Refresh
