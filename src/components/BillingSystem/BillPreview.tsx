...
const BillPreview: React.FC<BillPreviewProps> = ({ billData, index }) => {
  ...
  return (
    <div className="force-light">
      <div
        className="card print:shadow-none relative min-h-[900px] pb-10 bg-white text-black"
        id={`bill-preview${index !== undefined ? `-${index}` : ''}`}
      >
            ...
            <div>
              <h3 className="text-xl font-semibold">
                {billData.formType === 'restaurant'
                  ...
                  : billData.formType === 'hotel'
                    ? 'Guest Information'
                    : billData.formType === 'travel'
                      ? 'Agent Information'
                      : 'Guest Information'}
              </h3>
        ...
        {billData.formType !== 'restaurant' && (
          <div className="mb-6">
            <h3 className="text-xl font-semibold border-b border-gray-200 pb-2 mb-3">
                ...
                <tbody>
                  {billData.doubleBedRoom > 0 && (
                    <tr className="border-b border-gray-100">
                      ...
                      <td className="py-2 px-3">
                        {formatCurrency(billData.doubleBedRate)}
                      </td>
                      <td className="py-2 px-3">{billData.days}</td>
                      <td className="py-2 px-3">
                        {formatCurrency(
                          billData.doubleBedRoom *
                            billData.doubleBedRate *
                            billData.days
                        )}
                      </td>
                    </tr>
                  ...
                  {billData.tripleBedRoom > 0 && (
                    <tr className="border-b border-gray-100">
                      ...
                      <td className="py-2 px-3">
                        {formatCurrency(billData.tripleBedRate)}
                      </td>
                      <td className="py-2 px-3">{billData.days}</td>
                      <td className="py-2 px-3">
                        {formatCurrency(
                          billData.tripleBedRoom *
                            billData.tripleBedRate *
                            billData.days
                        )}
                      </td>
                    </tr>
                  ...
                  {billData.fourBedRoom > 0 && (
                    <tr className="border-b border-gray-100">
                      ...
                      <td className="py-2 px-3">
                        {formatCurrency(billData.fourBedRate)}
                      </td>
                      <td className="py-2 px-3">{billData.days}</td>
                      <td className="py-2 px-3">
                        {formatCurrency(
                          billData.fourBedRoom *
                            billData.fourBedRate *
                            billData.days
                        )}
                      </td>
                    </tr>
                  ...
                  {billData.extraBedRoom > 0 && (
                    <tr className="border-b border-gray-100">
                      ...
                      <td className="py-2 px-3">
                        {formatCurrency(billData.extraBedRate)}
                      </td>
                      <td className="py-2 px-3">{billData.days}</td>
                      <td className="py-2 px-3">
                        {formatCurrency(
                          billData.extraBedRoom *
                            billData.extraBedRate *
                            billData.days
                        )}
                      </td>
                    </tr>
                  ...
                  {billData.kitchenRoom > 0 && (
                    <tr className="border-b border-gray-100">
                      ...
                      <td className="py-2 px-3">
                        {formatCurrency(billData.kitchenRate)}
                      </td>
                      <td className="py-2 px-3">{billData.days}</td>
                      <td className="py-2 px-3">
                        {formatCurrency(
                          billData.kitchenRoom *
                            billData.kitchenRate *
                            billData.days
                        )}
                      </td>
                    </tr>
                  ...
                  <tr className="bg-blue-50 font-medium">
                    ...
                    <td className="py-2 px-3">
                      {formatCurrency(billData.billAmount)}
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            <ol className=" pl-5 text-sm space-y-2">
              <li>1.Check out time 10:00AM</li>
              <li>2.Cheques are not accepted.</li>
              <li>3.Bill must be paid on presentation</li>
              <li>4.Disputes are subject to uttarkashi jurisdiction only.</li>
              <li>
        ...
        <div className="mt-8 border-t border-gray-200 pt-4 text-center text-sm text-gray-500">
          <p>
            Thank you for choosing {billData.hotel}. We hope to see you again!
          </p>
        </div>
        <div
          className="absolute bottom-0 left-1/2 transform -translate-x-1/2 h-1 w-32 
                    bg-gradient-to-r from-[var(--icon-bg-indigo)] to-[var(--icon-bg-purple)] rounded-full"
        ></div>
      </div>
    </div>
  );
